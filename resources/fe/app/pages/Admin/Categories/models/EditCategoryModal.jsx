...
import {updateCategory} from "../../../../redux/action/categoryAction.js";

const EditCategoryModal = ({ isOpen, onClose, inputItem }) => {
    ...
    const handleClosing = () => {
        setForm({ name: '', description: '', type: "Food" });
        onClose();
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!inputItem || !inputItem.id) {
            notify({group: "foo", title: "Error", text: "Invalid category data."}, 2000);
            return;
        }
        try {
            await dispatch(updateCategory(inputItem.id, form));
            notify({group: "foo", title: "Success", text: "Category edited successfully!"}, 4000);
            setForm({name: '', description: '', type: 'Food'});
        } catch (error) {
            notify({group: "foo", title: "Error", text: "An error occurred while editing the category."}, 2000);
            console.error(error);
        }
